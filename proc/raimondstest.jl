...
df = readtable("$(dirname(@__FILE__))/raimonds/uabrotminkap9.dat",separator='\t',header=false)
...
df = readtable("$(dirname(@__FILE__))/raimonds/ucbrotminkap9.dat",separator='\t',header=false)
