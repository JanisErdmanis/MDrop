...
pcs["CimursPC"] = PC("janiserdmanis@5.179.6.144","~/Documents/MDrop/",6)
...
function pushpc(pc)
    #run(`rsync -av --bwlimit=5 --exclude="figures/*" --exclude="*/.*" --exclude=".git/*" --exclude="*/METADATA/*" --exclude="*/.git/*" --info=progress2 $BaseDir/ $(pc.sshlogin):$(pc.workingdir)`)
    run(`rsync -a --exclude="figures/*" --exclude="*/.*" --exclude=".git/*" --exclude="*/METADATA/*" --exclude="*/.git/*" --info=progress2 $BaseDir/ $(pc.sshlogin):$(pc.workingdir)`)
end
